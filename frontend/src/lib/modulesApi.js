...
/**
 * Met à jour partiellement un module du tuteur connecté.
 * @param {number} moduleId
 * @param {object} body
 * @param {string} accessToken
 */
/**
 * Détail d'un module du tuteur connecté (JWT, propriétaire uniquement).
 * @param {number} moduleId
 * @param {string} accessToken
 */
export async function fetchTutorModuleById(moduleId, accessToken) {
  const base = getApiBase();
...
export async function patchTutorModule(moduleId, body, accessToken) {
  const base = getApiBase();
  const r = await fetch(`${base}/api/tuteur/modules/${moduleId}/`, {
    method: 'PATCH',
    headers: {
...
/**
 * Suppression logique d'un module du tuteur connecté (actif=false).
 * @param {number} moduleId
 * @param {string} accessToken
 */
export async function deleteTutorModule(moduleId, accessToken) {
  const base = getApiBase();
  const r = await fetch(`${base}/api/tuteur/modules/${moduleId}/`, {
    method: 'DELETE',
    headers: {
  ...
  if (!r.ok) {
    const msg =
      typeof data.detail === 'string'
        ...
        : r.status === 404
          ? 'Module introuvable.'
          : `Impossible de charger le module (HTTP ${r.status}).`;
