...
import { canStudentJoinOnlineMeeting } from '../../lib/reservationFormat';
...
export function buildTutorsFromLocalReservations(reservations, studentId) {
  ...
  for (const r of completed) {
    ...
    if (!byTutor[tid]) {
      ...
      const photo = resolveAvatarSrc(r.tutor_avatar ?? r.tutorAvatar) || resolveAvatarSrc(mt);
      byTutor[tid] = {
        tutorId: tid,
    ...
    } else if (!byTutor[tid].avatarUrl) {
      const photo = resolveAvatarSrc(r.tutor_avatar ?? r.tutorAvatar);
      if (photo) byTutor[tid].avatarUrl = photo;
    }
    ...
    <DashboardLayout>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
          {tutors.map((row) => {
            const avatarSrc =
              resolveAvatarSrc(row) ||
              resolveAvatarSrc(row.tutor_avatar ?? row.tutorAvatar ?? row.avatarUrl ?? row.avatar) ||
              undefined;
            return (
            <div key={row.tutorId} className="card hover:shadow-md transition-all">
              <div className="flex items-center gap-3 mb-4">
                <Avatar
                  initials={tutorInitials(row.tutorName)}
                  src={avatarSrc}
                  size="lg"
                  altText={row.tutorName || 'Tuteur'}
                />
