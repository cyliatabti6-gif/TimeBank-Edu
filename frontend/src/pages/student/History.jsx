  ...
  const rows = useMemo(() => {
    ...
    return [...sessionHistory]
      .map((h) => {
        ...
        const partnerAvatar = asStudent
          ? h.tutor_avatar ?? h.tutorAvatar ?? ''
          : h.student_avatar ?? h.studentAvatar ?? '';
        const avatarSrc = resolveAvatarSrc(partnerAvatar) || undefined;
        if (import.meta.env.DEV) {
          console.log('HISTORY AVATAR DEBUG:', {
            id: h.id,
            partnerName,
            partnerAvatar,
            avatarSrc,
                  ...
                  <tr key={h.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        <Avatar initials={h.avatar} src={h.avatarSrc} size="xs" />
