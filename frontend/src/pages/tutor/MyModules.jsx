...
import { Plus, Edit2, Eye, Trash2, BookOpen, Check, AlertCircle } from 'lucide-react';
import DashboardLayout from '../../components/layout/DashboardLayout';
import StarRating from '../../components/common/StarRating';
import { getAccessToken } from '../../lib/authStorage';
import { deleteTutorModule, fetchMyTutorModules } from '../../lib/modulesApi';
...
export default function MyModules() {
  ...
  const handleDeleteModule = useCallback(async (mod) => {
    const token = getAccessToken();
    if (!token) {
      setError('Connectez-vous pour supprimer un module.');
      return;
    }
    setBusyId(mod.id);
    setError('');
    try {
      await deleteTutorModule(mod.id, token);
      setModules((prev) => prev.filter((x) => x.id !== mod.id));
      pushToast(`Module « ${mod.title} » supprimé.`, 'success');
    } catch (e) {
      const msg = e?.message || 'Impossible de supprimer ce module.';
      setError(msg);
      pushToast(msg, 'error');
    } finally {
      setBusyId(null);
      ...
      <div className="fixed bottom-4 right-4 z-[120] flex flex-col gap-2 max-w-sm pointer-events-none">
        {toasts.map((t) => (
          ...
          >
            ...
            {t.type !== 'error' ? <Check size={18} className="flex-shrink-0 mt-0.5 text-emerald-600" /> : null}
            <span>{t.message}</span>
          </div>
        ))}
      </div>
          ...
          {modules.map((mod) => (
            <div key={mod.id} className="card hover:shadow-md transition-all">
              ...
              <div className="flex gap-2">
                <button
                  type="button"
                  onClick={() => navigate(`/tutor/modules/edit/${mod.id}`)}
                  disabled={busyId === mod.id}
                  className="flex-1 text-xs border border-gray-200 text-gray-600 py-1.5 rounded-lg flex items-center justify-center gap-1 hover:bg-gray-50 disabled:opacity-60 disabled:cursor-not-allowed"
                ...
                <button
                  ...
                  disabled={busyId === mod.id}
                  className="w-8 text-xs bg-red-50 text-red-500 py-1.5 rounded-lg flex items-center justify-center disabled:opacity-50 disabled:cursor-not-allowed"
