...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      login(data);
      const mapped = mapApiUserToAppUser(data.user);
      const from = location.state?.from;
      const safeFrom = typeof from === 'string' && from.startsWith('/') ? from : null;
      if (isPlatformAdmin(mapped)) navigate('/admin/dashboard');
      else if (safeFrom) navigate(safeFrom, { replace: true });
      else navigate(defaultDashboardPath(mapped));
    } catch {
