    ...
    <DashboardLayout>
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        <div className="card bg-gradient-to-br from-primary-50 to-primary-100 border-primary-200">
          ...
          <div className="text-2xl font-bold text-primary-700">{currentUser?.balance || 3}h</div>
      ...
      <div className="card">
        ...
        <div className="space-y-3">
          {mockSessions.map(s => {
            ...
            return (
              <div key={s.id} className="flex items-center gap-3 p-3 rounded-xl bg-gray-50 hover:bg-gray-100 transition-colors cursor-pointer">
                <div className="w-9 h-9 bg-primary-100 rounded-lg flex items-center justify-center flex-shrink-0">
                  {s.status === 'completed' ? <CheckCircle2 size={18} className="text-primary-600" /> :
                   s.status === 'in_progress' ? <PlayCircle size={18} className="text-orange-500" /> :
                   <Calendar size={18} className="text-blue-500" />}
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-gray-800 truncate">{s.module}</p>
                  <p className="text-xs text-gray-500">Avec {s.tutor}</p>
                </div>
      ...
      <div className="card mt-4">
        ...
        <div className="space-y-3">
          {[
            { module: 'Base de Données L3', tutor: 'Fatima Zahra', date: '20/05/2024', time: '16h-19h', status: 'Confirmée' },
            { module: 'Python L2', tutor: 'Ahmed Moussa', date: '22/05/2024', time: '18h-20h', status: 'En attente' },
          ].map((s, i) => (
            <div key={i} className="flex items-center gap-3 p-3 rounded-xl border border-gray-100">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-gray-500">Avec {s.tutor} • {s.date} {s.time}</p>
              </div>
              <span className={s.status === 'Confirmée' ? 'badge-blue' : 'badge-gray'}>{s.status}</span>
            </div>
          ))}
        </div>
      </div>
