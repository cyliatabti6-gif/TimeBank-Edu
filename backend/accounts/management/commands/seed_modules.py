...
Sans --with-demo-tutors : utilise uniquement les comptes déjà en rôle tuteur / both.
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        tutors = list(User.objects.filter(role__in=[Role.TUTOR, Role.BOTH]).order_by("id"))
        if not tutors:
            self.stdout.write(
                self.style.ERROR(
